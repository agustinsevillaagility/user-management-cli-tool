...
import { Auth0User, Auth0Organization, Auth0Role } from './types';
...
class Auth0OrganizationCLI {
  ...
  private organizations: Auth0Organization[] = [];
   ...
   * Find user by email
   */
  async findUser(email: string): Promise<Auth0User | null> {
    try {
   ...
   */
  async selectOrganizations(): Promise<Auth0Organization[]> {
   ...
   */
  async selectRoles(): Promise<Auth0Role[]> {
